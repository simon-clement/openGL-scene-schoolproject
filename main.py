...
def main():
    ...
    mon_pterosaure = load_skinned("pterosaur.dae")[0]
    viewer.add(Ptero(mon_pterosaure))
    viewer.add(Ptero(mon_pterosaure, 30, 60, 40, 0.5, 1))
    viewer.add(Ptero(mon_pterosaure, 190, 70, 50, 1.2, 4))
    viewer.add(Ptero(mon_pterosaure, 90, 20, 25, 1.5, 8))
    viewer.add(Ptero(mon_pterosaure, 300, 50, 30, 0.8, 5))
    viewer.add(Ptero(mon_pterosaure, 250, 45, 46, 1, 2))
    #viewer.add(Ptero(load_skinned("pterosaur.dae")[0], angle=0, distance=40, hauteur=20,  taille=1, decalage=0))
